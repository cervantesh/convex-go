...
| Offline realtime and sync semantics | Supported in default CI | `subscription_test.go`, `connection_state_test.go`, `subscription_soak_test.go`, [CONFORMANCE.md](CONFORMANCE.md) | Subscribe, auth callback refresh, connection state, unsubscribe, and deterministic reconnect behavior are covered offline. |
